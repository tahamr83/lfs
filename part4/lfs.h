...
public:
	...
	LogStructuredFileSystem(): block_size(1024),max_files(1000),max_file_size(8096),total_blocks(8192),max_file_name(128)
	{
	...
	int write_blocks(int start_block,int end_block,const char *buf,int size ,int offset,int fh)
	{
		...
		for(int blk=start_block;blk<=end_block;++blk)
        {
        	*debug<<"Block:"<<blk<<" Dirty:"<<dirty_map[inodes[fh].block[blk]]<<std::endl;
