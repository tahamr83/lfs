...
void init()
{
    ...
    int inum=0;
    //The starting block number for inode 0. Each file will have its first block allocated 
    //Since first 40 blocks are occupied by inode data the next 41th block will be assigned to the first file
    int sblock=41;
    ...
    {   
       ...
       // lseek(LOG_FP,(indoes[idx].block[start_block]*BLOCK_SIZE) + offset ,SEEK_SET);
        for(int blk=start_block;blk<=end_block;++blk)
        {
            ...
            bytes += read_block(LOG_FP,inodes[idx].block[blk] ,buf+bytes,rsize,blk_offset);
                     ...
                     off_t offset, struct fuse_file_info *fi)
{
    ...
    {   
       ...
       // lseek(LOG_FP,(indoes[idx].block[start_block]*BLOCK_SIZE) + offset ,SEEK_SET);
        ...
        {
            rsize=(std::min(BLOCK_SIZE,remaining_size)-blk_offset);
            bytes += write_block(LOG_FP,inodes[idx].block[blk] ,buf+bytes,rsize,blk_offset);
            blk_offset=0;//reset block offset for further 
            remaining_size-=bytes;
